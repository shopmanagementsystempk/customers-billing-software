...
const EditLedgerEntry = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setError('Failed to load ledger entry. Please try again.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(error.message || 'Failed to update ledger entry. Please try again.');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <>
      ...
      <Container className="pb-4">
        <PageHeader 
          ...
          subtitle="Update ledger transaction details."
        />
